...
class Annuaire (object):
    ...
    def _connect(self):
        if not MySQLdb:
            self._log(u"FATAL: no mysql found - Annuaire module not available")
            return False
        try:
            self._db = MySQLdb.connect(
                host=ANNUAIRE_DB_SERVER, db=ANNUAIRE_DB_NAME, user=ANNUAIRE_DB_USER, passwd=ANNUAIRE_DB_PASS)
        except MySQLdb.Error as e:
            if e[0] == 1130:
                self._log(
                    u'FATAL : can\'t connect to server : ' + unicode(e[1]))
                return False
    ...
    def _update_user(self, user_login):
        ...
        if u.main_team is not None:
            ...
            try:
                ...
                self._log(u'main team \'' + unicode(n) +
                          '\' can\'t be found in list of teams ' + unicode(equipes))
            else:
                del equipes[idx]
            equipes.insert(0, n)
        tags = [x.name for x in u.flags.all() if x.name ==
                settings.FLAG_PHOTO_WEB]
        ...
        if u.display_in_directory():
            if user is None:
                ...
                # calculate changes
                changes = dict((key, new[key])
                               for key in fields if (new[key] != user[key]))
                if len(changes) > 0:
                    ...
                    sql = 'update ' + ANNUAIRE_DB_TABLE + ' set ' + \
                        ','.join([k + '=%s' for k in keys]) + ' where id=%s;'
                    cursor.execute(sql, values)
                # no changes required
        else:
            if user is not None:
    ...
    def _update_photo(self, user_login):
        ...
        if u.photo_path is None or len(u.photo_path) == 0:
            ...
            return False
        spath = os.path.join(settings.USER_PHOTO_PATH,
                             str(u.uidnumber), u.photo_path)
        self._log(spath)
        ...
        a = af.rem()
        res = a.copy(PHOTO_SERVER, PHOTO_FILE_OWNER,
                     PHOTO_FILE_GROUP, spath, dpathl, PHOTO_FILE_MODE)
        if not res:
            a.log(u'FATAL: unable to copy file ' + unicode(spath) +
                  u' to ' + unicode(PHOTO_SERVER) + u':' + unicode(dpathl))
            return False
        ...
        spath_mini = f.name
        img.save(f, MINI_PHOTO_FORMAT, quality=MINI_PHOTO_QUALITY,
                 optimize=True, progressive=True)
        f.flush()
        # copy the temp file
        res = a.copy(PHOTO_SERVER, PHOTO_FILE_OWNER,
                     PHOTO_FILE_GROUP, spath_mini, dpaths, PHOTO_FILE_MODE)
        f.close()
        if not res:
    ...
    def UpdateUser(self, *args, **kwargs):
        if not MySQLdb:
            self._log("Annuaire.UpdateUser : MYSQL not loaded - Annuaire module disabled")
            # pretend we did the work, to avoid blocking other tasks
            return True
        _, command = args
    ...
    def AnnuaireUpdate(self, *args, **kwargs):
        if not MySQLdb:
            self._log("Annuaire.AnnuaireUpdate : MYSQL not loaded - Annuaire module disabled")
			# pretend we did the work, to avoid blocking other tasks
            return True
        self._init_logger(**kwargs)
        users = []
